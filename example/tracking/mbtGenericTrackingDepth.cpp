...
namespace {
  ...
  bool getOptions(int argc, const char **argv, std::string &ipath, std::string &configFile, std::string &configFile_depth,
    ...
    while ((c = vpParseArgv::parse(argc, argv, GETOPTARGS, &optarg_)) > 1) {
      ...
      case 'X': configFile_depth = optarg_; break;
      case 'm': modelFile = optarg_; break;
      case 'M': modelFile_depth = optarg_; break;
      case 'n': initFile = optarg_; break;
      case 'f': displayFeatures = false; break;
  ...
  void loadConfiguration(vpMbTracker * const tracker, const std::string &
    ...
    //Depth
    dynamic_cast<vpMbGenericTracker*>(tracker)->setDepthNormalFeatureEstimationMethod(vpMbtFaceDepthNormal::ROBUST_FEATURE_ESTIMATION);
...
int main(int argc, const char ** argv) {
  try {
    ...
    std::vector<int> trackerTypes = {trackerType_image, trackerType_depth};
...
#else
int main() {
  std::cerr << "visp_mbt, visp_gui modules and OpenCV are required to run this example." << std::endl;
  return EXIT_SUCCESS;
}
#endif

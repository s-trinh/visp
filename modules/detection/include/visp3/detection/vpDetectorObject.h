...
#ifndef __vpDetectorObject_h__
#define __vpDetectorObject_h__
...
public:
...
#if defined(VISP_HAVE_MODULE_TT)
  enum vpWarpType {
    WARP_AFFINE, WARP_HOMOGRAPHY, WARP_HOMOGRAPHY_SL3, WARP_SRT
  };
  enum vpTrackerType {
    TRACKER_SSD_INVERSE_COMPOSITIONAL, TRACKER_ZNCC_INVERSE_COMPOSITIONAL
  };
#endif
  ...
  vpObjectDetection() : m_boundingBox(), m_boundingBoxHistory(), m_statusHistory(),
#if defined(VISP_HAVE_MODULE_TT)
      m_trackingHasDiverged(false),
#endif
      m_update(false),m_maxVectorHistory(30)
#if defined(VISP_HAVE_MODULE_TT)
      , m_area_zone_cur(0.0), m_area_zone_prev(0.0), m_p(), m_target(), m_tracker(NULL),
      ...
      m_trackerParamThresholdGradient(60.0),
      m_trackerType(TRACKER_SSD_INVERSE_COMPOSITIONAL), m_warp(NULL), m_warpType(WARP_AFFINE), m_zone_cur(), m_zone_ref()
#endif
  {
#if defined(VISP_HAVE_MODULE_TT)
    initTrackingType();
  ...
  vpObjectDetection(const vpRect &r, const vpObjectStatus &status
#if defined(VISP_HAVE_MODULE_TT)
      , const vpTrackerType &trackerType=TRACKER_SSD_INVERSE_COMPOSITIONAL, const vpWarpType &warpType=WARP_AFFINE,
      const unsigned int iterationMax=100, const double lambda=0.001, const unsigned int pyramidLevels=2,
      const unsigned int pyramidLevelToStop=1, const int sample_i=2, const int sample_j=2,
      const double thresholdGradient=60.0
#endif
    ) : m_boundingBox(),
      m_boundingBoxHistory(), m_statusHistory(),
#if defined(VISP_HAVE_MODULE_TT)
      m_trackingHasDiverged(false),
#endif
      m_update(false), m_maxVectorHistory(30)
#if defined(VISP_HAVE_MODULE_TT)
      , m_area_zone_cur(0.0), m_area_zone_prev(0.0), m_p(), m_target(), m_tracker(NULL),
      m_trackerParamIterationMax(iterationMax), m_trackerParamLambda(lambda), m_trackerParamPyramidLevels(pyramidLevels),
      m_trackerParamPyramidLevelToStop(pyramidLevelToStop), m_trackerParamSample_i(sample_i),
      m_trackerParamSample_j(sample_j), m_trackerParamThresholdGradient(thresholdGradient),
      m_trackerType(trackerType), m_warp(NULL), m_warpType(warpType), m_zone_cur(), m_zone_ref()
#endif
  {
  ...
  vpObjectDetection(const vpObjectDetection &object) {
    ...
    m_trackerParamIterationMax = object.m_trackerParamIterationMax;
    m_trackerParamLambda = object.m_trackerParamLambda;
    m_trackerParamPyramidLevels = object.m_trackerParamPyramidLevels;
    m_trackerParamPyramidLevelToStop = object.m_trackerParamPyramidLevelToStop;
    m_trackerParamSample_i = object.m_trackerParamSample_i;
    m_trackerParamSample_j = object.m_trackerParamSample_j;
    m_trackerParamThresholdGradient = object.m_trackerParamThresholdGradient;

    m_trackerType = object.m_trackerType;
    m_warpType = object.m_warpType;
  ...
  vpObjectDetection &operator=(const vpObjectDetection &source) {
    if (&source != this) {
      ...
      m_trackerParamIterationMax = source.m_trackerParamIterationMax;
      m_trackerParamLambda = source.m_trackerParamLambda;
      m_trackerParamPyramidLevels = source.m_trackerParamPyramidLevels;
      m_trackerParamPyramidLevelToStop = source.m_trackerParamPyramidLevelToStop;
      m_trackerParamSample_i = source.m_trackerParamSample_i;
      m_trackerParamSample_j = source.m_trackerParamSample_j;
      m_trackerParamThresholdGradient = source.m_trackerParamThresholdGradient;

      m_trackerType = source.m_trackerType;
      m_warpType = source.m_warpType;
  ...
  */
  ...
  unsigned int getTrackerParamIterationMax() const {
    return m_trackerParamIterationMax;
  }

  double getTrackerParamLambda() const {
    return m_trackerParamLambda;
  }

  unsigned int getTrackerParamPyramidLevels() const {
    return m_trackerParamPyramidLevels;
  }

  unsigned int getTrackerParamPyramidLevelToStop() const {
    return m_trackerParamPyramidLevelToStop;
  }

  int getTrackerParamSample_i() const {
    return m_trackerParamSample_i;
  }

  int getTrackerParamSample_j() const {
    return m_trackerParamSample_j;
  }

  double getTrackerParamThresholdGradient() const {
    return m_trackerParamThresholdGradient;
  }
  ...
  /*!
    ...
    \return The type of the tracker.
  */
  vpTrackerType getTrackerType() const {
    return m_trackerType;
  }

  /*!
    Return the type of warping.

    \return The type of warping.
  */
  vpWarpType getWarpType() const {
    return m_warpType;
  }

  void setTrackerParamIterationMax(const unsigned int iterationMax) {
    m_trackerParamIterationMax = iterationMax;
    m_tracker->setIterationMax(m_trackerParamIterationMax);
  }

  void setTrackerParamLambda(const double lambda) {
    m_trackerParamLambda = lambda;
    m_tracker->setLambda(m_trackerParamLambda);
  }

  void setTrackerParamPyramidal(const unsigned nLevels, const unsigned int level_to_stop) {
    m_trackerParamPyramidLevels = nLevels;
    m_trackerParamPyramidLevelToStop = level_to_stop;
    m_tracker->setPyramidal(m_trackerParamPyramidLevels, m_trackerParamPyramidLevelToStop);
  }

  void setTrackerParamSampling(const int sample_i, const int sample_j) {
    m_trackerParamSample_i = sample_i;
    m_trackerParamSample_j = sample_j;
    m_tracker->setSampling(m_trackerParamSample_i, m_trackerParamSample_j);
  }

  void setTrackerParamThresholdGradient(const double thresholdGradient) {
    m_trackerParamThresholdGradient = thresholdGradient;
    m_tracker->setThresholdGradient(m_trackerParamThresholdGradient);
  }

  void setTrackingType(const vpTrackerType &trackerType, const vpWarpType &warpType) {
    if(m_tracker != NULL) {
      delete m_tracker;
      m_tracker = NULL;
    }
...
private:
  ...
  vpRect m_target;
  //! Template tracker
  vpTemplateTracker *m_tracker;
  //! Tracker parameter iteration max
  unsigned int m_trackerParamIterationMax;
  //! Tracker parameter lambda
  double m_trackerParamLambda;
  //! Tracker parameter number of pyramid levels
  unsigned int m_trackerParamPyramidLevels;
  //! Tracker parameter level to stop
  unsigned int m_trackerParamPyramidLevelToStop;
  //! Tracker parameter sample_i
  int m_trackerParamSample_i;
  //! Tracker parameter sample_j
  int m_trackerParamSample_j;
  //! Tracker parameter threshold gradient
  double m_trackerParamThresholdGradient;
  //! Template tracker type
  vpTrackerType m_trackerType;
  //! Warp type for the template tracker
  vpTemplateTrackerWarp *m_warp;
  //! Warp type
  vpWarpType m_warpType;
  ...
  void initTrackingType() {
    switch(m_warpType) {
    case WARP_AFFINE:
      m_warp = new vpTemplateTrackerWarpAffine;
      break;

    case WARP_HOMOGRAPHY:
      m_warp = new vpTemplateTrackerWarpHomography;
      break;

    case WARP_HOMOGRAPHY_SL3:
      m_warp = new vpTemplateTrackerWarpHomographySL3;
      break;

    case WARP_SRT:
      m_warp = new vpTemplateTrackerWarpSRT;
      break;

    default:
      m_warp = new vpTemplateTrackerWarpAffine;
      std::cerr << "Problem with m_warpType=" << m_warpType << std::endl;
      std::cerr << "Use by default vpTemplateTrackerWarpAffine !" << std::endl;
      break;
    }
    ...
    case TRACKER_ZNCC_INVERSE_COMPOSITIONAL:
      m_tracker = new vpTemplateTrackerZNCCInverseCompositional(m_warp);
      break;

    default:
      m_tracker = new vpTemplateTrackerSSDInverseCompositional(m_warp);
      std::cerr << "Problem with m_trackerType=" << m_trackerType << std::endl;
      std::cerr << "Use by default vpTemplateTrackerSSDInverseCompositional !" << std::endl;
      break;
    }
...
class VISP_EXPORT vpDetectorObject : public vpDetectorBase {
public:
  ...
  vpDetectorObject();
  vpDetectorObject(const std::string &classifierDataFile);

  virtual ~vpDetectorObject() {}


  void detect2(const vpImage<vpRGBa> &I, bool (*func)(const vpImage<vpRGBa> &I, const vpRect &boundingBox)=NULL,
      const double scaleFactor=1.1, const int minNeighbors=3, const cv::Size &minSize=cv::Size(),
      const cv::Size &maxSize=cv::Size());

  void detect2(const vpImage<unsigned char> &I, bool (*func)(const vpImage<unsigned char> &I, const vpRect &boundingBox)=NULL,
      const double scaleFactor=1.1, const int minNeighbors=3, const cv::Size &minSize=cv::Size(),
      const cv::Size &maxSize=cv::Size());
  ...
  */
  ...
  /*!
    Set the name of the object to detect for the message variable.

    \param name: Name of the object to detect.
  */
  void setMessageObjectName(const std::string &name) {
    m_messageObjectName = name;
  }
  ...
  */
  ...
  /*!
    ...
    \return The type of the tracker.
  */
  vpObjectDetection::vpTrackerType getTrackerType() const {
    return m_trackerType;
  }
  ...
  /*!
    ...
    \return The type of warping.
  */
  vpObjectDetection::vpWarpType getWarpType() const {
    return m_warpType;
  }
  ...
  void setTrackerParamIterationMax(const unsigned int iterationMax) {
    ...
    for(std::map<int, vpObjectDetection>::iterator it = m_mapOfDetectedObjects.begin();
      ...
      it->second.setTrackerParamIterationMax(m_trackerParamIterationMax);
    }
  }
  ...
  void setTrackerParamLambda(const double lambda) {
    ...
    for(std::map<int, vpObjectDetection>::iterator it = m_mapOfDetectedObjects.begin();
        it != m_mapOfDetectedObjects.end(); ++it) {
      it->second.setTrackerParamLambda(m_trackerParamLambda);
    }
  }
  ...
  void setTrackerParamPyramidal(const unsigned nLevels, const unsigned int level_to_stop) {
    ...
    for(std::map<int, vpObjectDetection>::iterator it = m_mapOfDetectedObjects.begin();
        it != m_mapOfDetectedObjects.end(); ++it) {
      it->second.setTrackerParamPyramidal(m_trackerParamPyramidLevels, m_trackerParamPyramidLevelToStop);
    }
  }
  ...
  void setTrackerParamSampling(const int sample_i, const int sample_j) {
    ...
    for(std::map<int, vpObjectDetection>::iterator it = m_mapOfDetectedObjects.begin();
        it != m_mapOfDetectedObjects.end(); ++it) {
      it->second.setTrackerParamSampling(m_trackerParamSample_i, m_trackerParamSample_j);
    }
  }
  ...
  void setTrackerParamThresholdGradient(const double thresholdGradient) {
    ...
    for(std::map<int, vpObjectDetection>::iterator it = m_mapOfDetectedObjects.begin();
        it != m_mapOfDetectedObjects.end(); ++it) {
      it->second.setTrackerParamThresholdGradient(m_trackerParamThresholdGradient);
    }
  }

  void setTrackingType(const vpObjectDetection::vpTrackerType &trackerType, const vpObjectDetection::vpWarpType &warpType) {
    m_trackerType = trackerType;
    m_warpType = warpType;

    for(std::map<int, vpObjectDetection>::iterator it = m_mapOfDetectedObjects.begin();
        it != m_mapOfDetectedObjects.end(); ++it) {
      it->second.setTrackingType(m_trackerType, m_warpType);
    }
  }
#endif
...
private:
  ...
  //! Maximal life time of an object when it is not detected by the cascade classifier (lost object)
  size_t m_maxLifeTime;
  //! Name of the object we want to detect for message
  std::string m_messageObjectName;
  ...
  double m_trackerParamThresholdGradient;
  //! Template tracker type
  vpObjectDetection::vpTrackerType m_trackerType;
  //! Warp type
  vpObjectDetection::vpWarpType m_warpType;
#endif
  ...
  std::string getMessageObjectName() const {
    return m_messageObjectName;
  }

...
vpMbDepthNormalTracker::vpMbDepthNormalTracker() :
  ...
  m_depthNormalHiddenFacesDisplay(), m_depthNormalI_dummyVisibility(), m_depthNormalListOfActiveFaces(), m_depthNormalListOfDesiredFeatures(),
  m_depthNormalFaces(), m_depthNormalPclPlaneEstimationMethod(2), m_depthNormalPclPlaneEstimationRansacMaxIter(200), m_depthNormalPclPlaneEstimationRansacThreshold(0.001),
  m_depthNormalSamplingStepX(2), m_depthNormalSamplingStepY(2), m_depthNormalUseRobust(false),
  m_error_depthNormal(), m_L_depthNormal(), m_robust_depthNormal(), m_w_depthNormal(), m_weightedError_depthNormal()
  //TODO: debug
  , m_debug(false)
{
  m_maxIter = 50;
...
void vpMbDepthNormalTracker::addFace(vpMbtPolygon &polygon, const bool alreadyClose) {
  ...
  if(nbpt > 0){
    for (unsigned int i = 0 ; i < nbpt-1 ; i++) {
      normal_face->addLine(polygon.p[i], polygon.p[i+1], &m_depthNormalHiddenFacesDisplay, polygon.getIndex(), polygon.getName());
    }

    if (!alreadyClose) {
      //Add last line that closes the face
      normal_face->addLine(polygon.p[nbpt-1], polygon.p[0], &m_depthNormalHiddenFacesDisplay, polygon.getIndex(), polygon.getName());
    }
  }
...
void vpMbDepthNormalTracker::computeVisibility(const unsigned int width, const unsigned int height) {
  ...
  if (useScanLine) {
    if (clippingFlag <= 2) {
      cam.computeFov(m_depthNormalI_dummyVisibility.getWidth(), m_depthNormalI_dummyVisibility.getHeight());
    }
...
void vpMbDepthNormalTracker::computeVVS() {
  ...
  while( std::fabs(normRes_1 - normRes) > m_stopCriteriaEpsilon && (iter < m_maxIter) ) {
    if (m_debug) {
      std::cerr << "\nIter: " << iter << " / " << m_maxIter << std::endl;

      std::cerr << "cMo:" << std::endl;
      for (unsigned int i = 0; i < cMo.getRows(); i++) {
        for (unsigned int j = 0; j < cMo.getCols(); j++) {
          std::cerr << std::setprecision(17) << cMo[i][j] << " ";
        }
        std::cerr << std::endl;
      }
    }
    ...
    if (!reStartFromLastIncrement) {
      ...
      if (m_debug) {
        std::cerr << "m_L_depth:" << std::endl;
        for (unsigned int i = 0; i < m_L_depthNormal.getRows(); i++) {
          for (unsigned int j = 0; j < m_L_depthNormal.getCols(); j++) {
            std::cerr << std::setprecision(17) << m_L_depthNormal[i][j] << " ";
          }
          std::cerr << std::endl;
        }
      }
      ...
      if (m_debug) {
        std::cerr << "m_error_depth:" << std::endl;
        for (unsigned int i = 0; i < m_error_depthNormal.getRows(); i++) {
          std::cerr << std::setprecision(17) << m_error_depthNormal[i] << std::endl;
        }

        std::cerr << "m_w_depth:" << std::endl;
        for (unsigned int i = 0; i < m_w_depthNormal.getRows(); i++) {
          std::cerr << std::setprecision(17) << m_w_depthNormal[i] << std::endl;
        }
      }

      computeVVSPoseEstimation(isoJoIdentity_, iter, m_L_depthNormal, LTL, m_weightedError_depthNormal, m_error_depthNormal, error_prev, LTR, mu, v);

      cMo_prev = cMo;
      cMo =  vpExponentialMap::direct(v).inverse() * cMo;

      if (m_debug) {
        std::cerr << "v=" << std::setprecision(17) << v[0] << " ; " << v[1] << " ; " << v[2] << " ; " << v[3] << " ; " << v[4] << " ; " << v[5] << std::endl;
...
void vpMbDepthNormalTracker::computeVVSInit() {
  ...
  m_error_depthNormal.resize(nb_features);
...
void vpMbDepthNormalTracker::computeVVSInteractionMatrixAndResidu() {
  ...
  for (std::vector<vpMbtFaceDepthNormal*>::const_iterator it = m_depthNormalListOfActiveFaces.begin(); it != m_depthNormalListOfActiveFaces.end(); ++it) {
    ...
    vpColVector face_error = features_face - m_depthNormalListOfDesiredFeatures[cpt];
...
void vpMbDepthNormalTracker::display(const vpImage<unsigned char> &I, const vpHomogeneousMatrix &cMo_, const vpCameraParameters &cam_,
  ...
  if (useScanLine) {
    if (getClipping() > 3) // Contains at least one FOV constraint
...
void vpMbDepthNormalTracker::display(const vpImage<vpRGBa> &I, const vpHomogeneousMatrix &cMo_, const vpCameraParameters &cam_,
                                     const vpColor &col, const unsigned int thickness, const bool displayFullModel) {
  vpCameraParameters c = cam_;
...
void vpMbDepthNormalTracker::reInitModel(const vpImage<unsigned char> &I, const std::string &cad_name, const vpHomogeneousMatrix &cMo_, const bool verbose) {
  cMo.eye();
...
void vpMbDepthNormalTracker::reInitModel(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &point_cloud, const std::string &cad_name, const vpHomogeneousMatrix &cMo_, const bool verbose) {
...
#if defined(VISP_HAVE_PCL)
void vpMbDepthNormalTracker::setPose(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &point_cloud, const vpHomogeneousMatrix &cdMo) {
  vpImage<unsigned char> I_dummy(point_cloud->height, point_cloud->width);
  cMo = cdMo;
  init(I_dummy);
}
...
void vpMbDepthNormalTracker::segmentPointCloud(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &point_cloud) {
  ...
  for (std::vector<vpMbtFaceDepthNormal*>::iterator it = m_depthNormalFaces.begin(); it != m_depthNormalFaces.end(); ++it) {
    ...
    if (face->isVisible()) {
      ...
      if (face->computeDesiredFeatures(cMo, point_cloud->width, point_cloud->height, point_cloud, desired_features, m_depthNormalSamplingStepX, m_depthNormalSamplingStepY)) {
...
void vpMbDepthNormalTracker::segmentPointCloud(const std::vector<vpColVector> &point_cloud, const unsigned int width, const unsigned int height) {
  m_depthNormalListOfActiveFaces.clear();
  m_depthNormalListOfDesiredFeatures.clear();
  ...
  for (std::vector<vpMbtFaceDepthNormal*>::iterator it = m_depthNormalFaces.begin(); it != m_depthNormalFaces.end(); ++it) {
    ...
    if (face->isVisible()) {
      ...
      if (face->computeDesiredFeatures(cMo, width, height, point_cloud, desired_features, m_depthNormalSamplingStepX, m_depthNormalSamplingStepY)) {
        m_depthNormalListOfDesiredFeatures.push_back(desired_features);
        m_depthNormalListOfActiveFaces.push_back(face);
      }
    }
...
void vpMbDepthNormalTracker::track(const std::vector<vpColVector> &point_cloud, const unsigned int width, const unsigned int height) {
...
void vpMbDepthNormalTracker::initCircle(const vpPoint& /*p1*/, const vpPoint &/*p2*/, const vpPoint &/*p3*/, const double /*radius*/,
...
void vpMbDepthNormalTracker::initCylinder(const vpPoint& /*p1*/, const vpPoint &/*p2*/, const double /*radius*/, const int /*idFace*/,

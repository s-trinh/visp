    ...
    if (!reStartFromLastIncrement) {
      ...
      if (m_debug) {
        ...
        for (unsigned int i = 0; i < m_L_depthDense.getRows(); i++) {
          for (unsigned int j = 0; j < m_L_depthDense.getCols(); j++) {
            std::cerr << std::setprecision(17) << m_L_depthDense[i][j] << " ";
          }
          std::cerr << std::endl;
        }
      }
      ...
      if (m_debug) {
        std::cerr << "m_error_depthDense:" << std::endl;
        for (unsigned int i = 0; i < m_error_depthDense.getRows(); i++) {
          std::cerr << std::setprecision(17) << m_error_depthDense[i] << std::endl;
        }
      }
...
void vpMbDepthDenseTracker::computeVVSInit() {
  ...
  m_error_depthDense.resize(m_denseDepthNbFeatures);

...
*/
void vpMbtFaceDepthDense::addLine(vpPoint &P1, vpPoint &P2, vpMbHiddenFaces<vpMbtPolygon> * const faces, int polygon, std::string name) {
...
#ifdef VISP_HAVE_PCL
bool vpMbtFaceDepthDense::computeDesiredFeatures(const vpHomogeneousMatrix &cMo, const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &point_cloud,
...
bool vpMbtFaceDepthDense::computeDesiredFeatures(const vpHomogeneousMatrix &cMo, const unsigned int width, const unsigned int height,
...
void vpMbtFaceDepthDense::computeInteractionMatrixAndResidu(const vpHomogeneousMatrix &cMo, vpMatrix &L, vpColVector &error) {
  if (m_pointCloudFace.empty()) {
    L.resize(0,0);
    error.resize(0);
  }

  L.resize(getNbFeatures(), 6);
  error.resize(getNbFeatures());
  ...
  if (getNbFeatures() >= 2) {
    ...
    const __m128d vnx = _mm_set_pd(nx, nx); //_mm_set_pd1 is not defined with MSVC
    const __m128d vny = _mm_set_pd(ny, ny);
    const __m128d vnz = _mm_set_pd(nz, nz);
    const __m128d vd  = _mm_set_pd(D, D);
  ...
  for (; cpt < m_pointCloudFace.size(); cpt+=3) {
    ...
    //L
    L[cpt/3][0] = nx;  L[cpt/3][1] = ny;  L[cpt/3][2] = nz; L[cpt/3][3] = _a1;  L[cpt/3][4] = _a2;  L[cpt/3][5] = _a3;
...
void vpMbtFaceDepthDense::displayFeature(const vpImage<unsigned char> &/*I*/, const vpHomogeneousMatrix &/*cMo*/, const vpCameraParameters &/*cam*/,
                                         const double /*scale*/, const unsigned int /*thickness*/) {
}

void vpMbtFaceDepthDense::displayFeature(const vpImage<vpRGBa> &/*I*/, const vpHomogeneousMatrix &/*cMo*/, const vpCameraParameters &/*cam*/,
                                         const double /*scale*/, const unsigned int /*thickness*/) {
}

...
#include <visp3/mbt/vpMbSiftTracker.h>
...
vpMbSiftTracker::vpMbSiftTracker()
  :
...
void vpMbSiftTracker::init(const vpImage<unsigned char> &I)
{
  if (!modelInitialised) {
...
void vpMbSiftTracker::reinit(const vpImage<unsigned char> &I)
{
  c0Mo = m_cMo;
  ctTc0.eye();
...
*/
void vpMbSiftTracker::resetTracker()
{
  m_cMo.eye();
...
*/
std::vector<vpImagePoint> vpMbSiftTracker::getKltImagePoints() const
{
...
*/
std::map<int, vpImagePoint> vpMbSiftTracker::getKltImagePointsWithId() const
{
...
*/
void vpMbSiftTracker::setCameraParameters(const vpCameraParameters &cam)
{
...
void vpMbSiftTracker::setPose(const vpImage<unsigned char> *const I, const vpImage<vpRGBa> *const I_color,
...
*/
void vpMbSiftTracker::setPose(const vpImage<unsigned char> &I, const vpHomogeneousMatrix &cdMo)
{
  vpMbSiftTracker::setPose(&I, nullptr, cdMo);
}
...
/*!
  ...
  \param cdMo : Pose to affect.
*/
void vpMbSiftTracker::setPose(const vpImage<vpRGBa> &I_color, const vpHomogeneousMatrix &cdMo)
{
  vpMbSiftTracker::setPose(nullptr, &I_color, cdMo);
}
...
/*!
  ...
  tracked.
*/
void vpMbSiftTracker::initFaceFromCorners(vpMbtPolygon &polygon)
{
...
/*!
  ...
  tracked.
*/
void vpMbSiftTracker::initFaceFromLines(vpMbtPolygon &polygon)
{
  vpMbtDistanceKltPoints *kltPoly = new vpMbtDistanceKltPoints();
  kltPoly->setCameraParameters(m_cam);
  kltPoly->polygon = &polygon;
...
*/
void vpMbSiftTracker::preTracking(const vpImage<unsigned char> &I)
{
  vpImageConvert::convert(I, cur);
  tracker.track(cur);
...
*/
bool vpMbSiftTracker::postTracking(const vpImage<unsigned char> &I, vpColVector &w)
{
...
*/
void vpMbSiftTracker::computeVVS()
{
  ...
  vpMbSiftTracker::computeVVSInit();

  while ((static_cast<int>((normRes - normRes_1) * 1e8) != 0) && (iter < m_maxIter)) {
    vpMbSiftTracker::computeVVSInteractionMatrixAndResidu();
...
void vpMbSiftTracker::computeVVSInit()
{
...
void vpMbSiftTracker::computeVVSInteractionMatrixAndResidu()
{
  unsigned int shift = 0;
  vpHomography H;
...
*/
void vpMbSiftTracker::track(const vpImage<unsigned char> &I)
{
  preTracking(I);
...
*/
void vpMbSiftTracker::track(const vpImage<vpRGBa> &I_color)
{
  vpImageConvert::convert(I_color, m_I);
  preTracking(m_I);
...
<conf>
  ...
  \endcode
*/
void vpMbSiftTracker::loadConfigFile(const std::string &configFile, bool verbose)
{
...
// Load projection error config
  ...
  (void)configFile;
  (void)verbose;
  throw(vpException(vpException::ioError, "vpMbSiftTracker::loadConfigFile() needs pugixml built-in 3rdparty"));
#endif
}
...
*/
void vpMbSiftTracker::display(const vpImage<unsigned char> &I, const vpHomogeneousMatrix &cMo,
                             const vpCameraParameters &cam, const vpColor &col, unsigned int thickness,
                             bool displayFullModel)
{
  std::vector<std::vector<double> > models =
    vpMbSiftTracker::getModelForDisplay(I.getWidth(), I.getHeight(), cMo, cam, displayFullModel);
...
*/
void vpMbSiftTracker::display(const vpImage<vpRGBa> &I, const vpHomogeneousMatrix &cMo, const vpCameraParameters &cam,
...
std::vector<std::vector<double> > vpMbSiftTracker::getFeaturesForDisplayKlt()
{
...
*/
std::vector<std::vector<double> > vpMbSiftTracker::getModelForDisplay(unsigned int width, unsigned int height,
...
*/
void vpMbSiftTracker::testTracking()
{
...
*/
void vpMbSiftTracker::initCylinder(const vpPoint &p1, const vpPoint &p2, double radius, int idFace,
...
*/
void vpMbSiftTracker::initCircle(const vpPoint &p1, const vpPoint &p2, const vpPoint &p3, double radius, int idFace,
                                const std::string &name)
{
  (void)idFace;
...
/*!
  ...
  \param name : The optional name of the circle.
*/
void vpMbSiftTracker::addCircle(const vpPoint &p1, const vpPoint &p2, const vpPoint &p3, double radius,
                               const std::string &name)
{
...
*/
void vpMbSiftTracker::reInitModel(const vpImage<unsigned char> &I, const std::string &cad_name,
...
*/
void vpMbSiftTracker::setUseKltTracking(const std::string &name, const bool &useKltTracking)
{
  for (std::list<vpMbtDistanceKltPoints *>::const_iterator it = kltPolygons.begin(); it != kltPolygons.end(); ++it) {
    vpMbtDistanceKltPoints *kltpoly = *it;
...
// Work around to avoid warning: libvisp_mbt.a(vpMbSiftTracker.cpp.o) has no symbols
void dummy_vpMbSiftTracker() { }

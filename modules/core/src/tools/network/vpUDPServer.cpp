...
int vpUDPServer::receive(std::string &msg, std::string &hostInfo, const int timeoutMs) {
  ...
  if (retval > 0) {
...
#if !defined(_WIN32) && (defined(__unix__) || defined(__unix) || (defined(__APPLE__) && defined(__MACH__))) // UNIX
    int length = recvfrom(m_socketFileDescriptor, m_buf, sizeof(m_buf) - 1, 0, (struct sockaddr *) &m_clientAddress, (socklen_t *)&m_clientLength);
#else
    int length = recvfrom(m_socketFileDescriptor, m_buf, sizeof(m_buf) - 1, 0, (struct sockaddr *) &m_clientAddress, &m_clientLength);
#endif
    if (length <= 0) {
      return length < 0 ? -1 : 0;
    }
    ...
    DWORD dwRetval = getnameinfo((struct sockaddr *) &m_clientAddress,
...
#if !defined(_WIN32) && (defined(__unix__) || defined(__unix) || (defined(__APPLE__) && defined(__MACH__))) // UNIX
  return sendto(m_socketFileDescriptor, m_buf, strlen(m_buf), 0, (struct sockaddr *) &m_clientAddress, m_clientLength);
#else
  return sendto(m_socketFileDescriptor, m_buf, (int)strlen(m_buf), 0, (struct sockaddr *) &m_clientAddress, m_clientLength);
#endif
}

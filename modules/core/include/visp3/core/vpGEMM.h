...
class VISP_EXPORT vpBLAS {
public:
    vpBLAS();
    ~vpBLAS();

    void dgemm(const vpArray2D<double>& A, const vpArray2D<double>& B, double alpha,
               vpArray2D<double>& C, double beta, int ops=0);
    void dgemv(const vpMatrix& A, double alpha, const vpColVector& x, double beta,
               vpColVector& y, bool transA=false);

private:
    vpBLAS(const vpBLAS&);              // noncopyable
    vpBLAS& operator=(const vpBLAS&);   //
